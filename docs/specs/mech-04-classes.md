...
Resource type is a runtime toggle (`PlayerStats.ChangeResourceRpc`) with no
persistent identity, which means a player currently holds abilities from three
different classes and can only use one third of their bar at a time. This is scaffold
...
| `M4-F-004` | Resource type is owned by the class and cannot be switched at runtime; `ChangeResourceRpc` is removed or restricted to debug builds | PlayMode: `M4_F_004_ResourceTypeIsImmutable` |
...
| ID | Requirement | Acceptance |
|---|---|---|
| `M4-N-001` | Class is server-validated at spawn; a client cannot claim a class it did not select or change class mid-session | PlayMode: `M4_N_001_ClassCannotBeForged` |
| `M4-N-002` | A client cannot use an ability outside its class's loadout, even by sending a valid slot index | PlayMode: `M4_N_002_CrossClassAbilityRejected` |
...
|---|---|---|---|
| M4-D1 | The launch class roster | (a) **three** — Warrior (rage), Mage (mana), Rogue (energy): one per resource system, all three already have a working ability as a seed, smallest path to "classes feel different"; (b) four, adding a Priest/healer (mana) so group play has a role split; (c) more | **Yes** |
| M4-D2 | Is there a healer at launch? A healer only matters if group content does | yes / no | Depends on M4-D1 |
...
classes is the smallest roster that proves the system. A healer is worth adding in
Phase 4 alongside the group content that would justify it.
...
a shared low-level consumable-style ability under option (a).
